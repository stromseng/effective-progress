...
- [ ] CJK/wide-character aware width calculation (`wcwidth`-style) instead of codepoint count.
  - Currently width calculations use codepoint count (`Array.from(text).length`). A CJK character like `你` counts as 1 but renders as 2 terminal columns. Descriptions containing CJK, emoji, or other wide characters will overflow their allocated column width, breaking the layout.
  ...
  - Every width call site in the Ink layout/format pipeline needs to use the new function.
