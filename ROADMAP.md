...
- make `SNAPSHOT_PUBLISH_INTERVAL_MILLIS` and other rendering constants etc be effect Configs. also `ETA_SAMPLE_WINDOW_MILLIS` and `ETA_SAMPLE_MAX_LENGTH`
...
- [ ] Half-character bar precision using `╸`/`╺` Unicode blocks for 2x visual resolution (Rich-inspired).
  - Currently `ProgressBarSegments` in `src/columns/bar.tsx` rounds each segment end with `Math.round(ratio * width)` - whole-character precision. On a 40-char bar each jump is 2.5%, visibly choppy for slow tasks.
...
- [ ] Smooth boundary transition chars between filled and empty bar regions.
  ...
  - Largely a freebie if half-character precision is implemented first - the two items share the same render logic change in `ProgressBarSegments`.
- [ ] Animated pulse bar for indeterminate tasks (scrolling gradient instead of bare spinner).
  - Currently `ProgressBarSegments` renders blank space for indeterminate tasks. The only visual indicator is the braille spinner (`⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏`) that `TaskIndicatorGlyph` renders in the description column.
  ...
  - `ProgressBarSegments` needs an indeterminate branch instead of the early return on `!isDeterminate(task)`.
